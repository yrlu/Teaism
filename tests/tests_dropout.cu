...
void test_dropout_cpu() {
  printf("Begin test dropout layer CPU\n");
  ...
  Dropout<float> dropout_layer(0.5);
  dropout_layer.Forward(bottom, top);
...
void test_dropout_gpu() {
  printf("Begin test dropout layer GPU\n");
  Session* session = Session::GetNewSession();
  session->gpu = true;
  session->batch_size = 2;
