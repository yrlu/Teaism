...
void test_reshape_cpu() {
  ...
  printf("%d %d %d %d\n", tensor_cpu->GetDims()[0], 
    tensor_cpu->GetDims()[1], tensor_cpu->GetDims()[2], tensor_cpu->GetDims()[3]);
}


__global__ show_dims(Tensor<float> *tensor_gpu) {
  printf("%d %d %d %d\n", tensor_gpu->GetDims()[0], 
    tensor_gpu->GetDims()[1], tensor_gpu->GetDims()[2], tensor_gpu->GetDims()[3]); 
}
...
void test_reshape_gpu() {
  ...
  show_dims<<<1, 1>>>show_dims(tensor_gpu)
  cudaFree(tensor_gpu);
