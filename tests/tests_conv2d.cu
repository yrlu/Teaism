...
void test_conv2d_gpu_valid() {
  ...
  const char* OUTPUT_BMP_PATH = "./tmp/test/out_gpu_valid.bmp";
...
void test_conv2d_gpu_same() {
  ...
  size_t stride = 2;
  // inputs: filter_height, filter_width, in_channels, out_channels, stride
  Conv2D<float> * conv_layer = new Conv2D<float>(kernel,kernel,1,1,stride,new GaussianKernelInitializer<float>(kernel), SAME);
  const char* OUTPUT_BMP_PATH = "./tmp/test/out_gpu_same.bmp";
...
int main() {
  test_conv2d_cpu();
  test_conv2d_gpu_valid();
  test_conv2d_gpu_same();
}
